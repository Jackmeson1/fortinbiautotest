...
def browser():
    ...
    control = BrowserControl(
        profile_path=r"C:\Users\test\AppData\Local\Google\Chrome\User Data\Default"
    )  # 替换为实际路径
    yield control
...
def test_search_bar_functionality(browser):
    ...
    assert (
        "Canada" in browser.get_page_title()
    ), "Search results page title does not contain 'Canada'"
...
def test_is_element_present_nonexistent(browser):
    ...
    assert not browser.is_element_present(
        By.ID, "nonexistentelement"
    ), "Non-existent element was found"
...
def test_get_page_source(browser):
    ...
    assert (
        "Example Domain" in page_source
    ), "Page source does not contain expected content"
...
def test_multiple_navigation(browser):
    ...
    assert browser.is_page_loaded(
        "Example Domain"
    ), "Example.com did not load correctly"
    browser.navigate_to("https://www.google.ca")
    assert browser.is_page_loaded(
        "Google"
...
def test_firefox_initialization():
    ...
    assert firefox_control.is_page_loaded(
        "Google"
    ), "Google homepage did not load correctly in Firefox"
