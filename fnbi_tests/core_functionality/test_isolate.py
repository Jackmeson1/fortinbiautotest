...
def test_isolate_navigation(browser, fnbi_app, fnbi_service):
    print("\n=== Starting test_isolate_navigation ===")
    window_title = "Chrome"
    api_key = os.getenv('OPENAI_API_KEY')
    example_image_path = os.path.join(project_root, 'resources', 'baseline_images', 'isolate_example.png')
    function_type = "isolate"
    ...
    try:
        ...
        isolate_url = config['test']['isolate_url']
        print(f"Trying to navigate to isolate URL: {isolate_url}")
        ...
        try:
            # Perform AI analysis on the screenshot
            ...
            # Core assertions for AI analysis
            assert result['functionality_match'] == True, "AI analysis indicates functionality does not match"
            assert result['confidence'] >= 0.8, f"AI analysis confidence ({result['confidence']}) is below threshold (0.8)"
        ...
        try:
            # Try to get current URL (might fail if page is isolated)
            current_url = browser.driver.current_url
        except WebDriverException as e:
            print(f"\nExpected WebDriver Error: {str(e)}")
            if 'target frame detached' in str(e):
                print("✓ This is the expected behavior when page is isolated")
